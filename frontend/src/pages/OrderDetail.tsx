...
export default function OrderDetail({ role }: OrderDetailProps) {
  ...
  return (
    <Page
      ...
      primaryAction={
        canFulfill
          ? { content: 'Add Fulfillment', onAction: () => setFulfillModal(true) }
          : undefined
      }
      secondaryActions={
        canAccept
          ...
          <Card>
            <BlockStack gap="400">
              ...
              <InlineStack gap="800">
                ...
                <BlockStack gap="100">
                  <Text as="span" variant="bodySm" tone="subdued">Wholesale Total</Text>
                  <Text as="span" variant="bodyMd">${order.total_wholesale_amount.toFixed(2)} {order.currency}</Text>
                </BlockStack>
                <BlockStack gap="100">
        ...
        <Layout.Section variant="oneHalf">
          <Card>
            <BlockStack gap="300">
              ...
              <Text as="p" variant="bodyMd">{order.customer_shipping_name}</Text>
              ...
              {order.customer_email && (
                <Text as="p" variant="bodySm">{order.customer_email}</Text>
              )}
              {order.customer_phone && (
                <Text as="p" variant="bodySm">{order.customer_phone}</Text>
              )}
            </BlockStack>
          </Card>
        </Layout.Section>
        ...
        <Layout.Section variant="oneHalf">
          <Card>
            <BlockStack gap="300">
              ...
              <Text as="p" variant="bodyMd">{order.notes || 'No notes'}</Text>
            </BlockStack>
          </Card>
        </Layout.Section>
            ...
            <BlockStack gap="400">
              ...
              {order.items && order.items.length > 0 ? (
                <DataTable
                  ...
                  headings={['Product', 'SKU', 'Qty', 'Unit Price', 'Fulfillment', 'Fulfilled']}
            ...
            <Card>
              <BlockStack gap="400">
                ...
                <DataTable
                  ...
                  headings={['Tracking', 'Company', 'Status', 'Synced', 'Synced At', 'Date']}
                  rows={fulfillments.map((f) => [
                    f.tracking_url ? (
                      <a href={f.tracking_url} target="_blank" rel="noopener noreferrer" key={f.id}>
                        {f.tracking_number}
                      </a>
                    ) : (
                      f.tracking_number
                    ...
                    f.synced_to_reseller ? <Badge tone="success">Yes</Badge> : <Badge>No</Badge>,
                    f.synced_at ? new Date(f.synced_at).toLocaleString() : '-',
